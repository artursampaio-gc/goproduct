...
export default function DashboardLayout() {
  ...
  // Load the dashboard layout from local storage with auto-migration for custom widgets
  useEffect(() => {
    if (availableWidgets.loaded) {
      let currentWidgets = [...remoteWidgets].filter(
        (w) => w !== 'gstart' && w !== 'news'
      );
      let currentLayouts = { ...remoteLayouts };

      if (currentLayouts.lg) {
        currentLayouts.lg = currentLayouts.lg.filter(
          (item: any) => item.i !== 'gstart' && item.i !== 'news'
        );
      }

      if (!currentWidgets.includes('create-order-doc')) {
        currentWidgets.push('create-order-doc');
      }
      if (!currentWidgets.includes('register-product')) {
        currentWidgets.push('register-product');
      }

      const lgLayout = currentLayouts.lg || [];
      if (!lgLayout.some((item: any) => item.i === 'create-order-doc')) {
        lgLayout.push({
          w: 6, h: 4, x: 0, y: 0, i: 'create-order-doc',
          minW: 3, minH: 2, moved: false, static: false
        });
      }
      if (!lgLayout.some((item: any) => item.i === 'register-product')) {
        lgLayout.push({
          w: 6, h: 4, x: 6, y: 0, i: 'register-product',
          minW: 3, minH: 2, moved: false, static: false
        });
      }
      currentLayouts.lg = lgLayout;

      setRemoteWidgets(currentWidgets);
      setRemoteLayouts(currentLayouts);

      setLayouts(currentLayouts);
      setWidgets(
        availableWidgets.items.filter((widget) =>
          currentWidgets.includes(widget.label)
        )
      );
  ...
  const loadWigs = ['create-order-doc', 'register-product'];
