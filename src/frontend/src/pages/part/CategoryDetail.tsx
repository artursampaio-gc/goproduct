  ...
  const panels: PanelType[] = useMemo(
    () => [
        ...
        label: !id
          ...
          : category?.level === 0
            ...
            : category?.level === 1
              ...
              : t`Subcategories`,
        icon: <IconSitemap />,
        // A Modelo (level 2) is the deepest level: it has no subcategories.
        hidden: !!id && category?.level === 2,
        content: (
          <PartCategoryTable parentId={id} parentLevel={category?.level} />
        )
      },
      SegmentedControlPanel({
        ...
        label: t`Produtos`,
        icon: <IconCategory />,
        // Products only live inside a Modelo (level 2); the global parts list
        // remains available at the root (no category selected).
        hidden: !!id && category?.level !== 2,
        selection: partsView,
