...
export function getNavTabs(user: UserStateProps): NavTab[] {
  ...
  // Abas ocultas temporariamente do menu superior.
  // Para reexibir uma aba, remova o nome correspondente desta lista
  // (ou esvazie a lista para mostrar todas novamente).
  const hiddenTabs = ['stock', 'manufacturing', 'sales'];

  return navTabs.filter((tab) => {
    if (hiddenTabs.includes(tab.name)) return false;
