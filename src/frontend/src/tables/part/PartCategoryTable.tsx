 ...
 */
export function PartCategoryTable({
  parentId,
  parentLevel
}: Readonly<{ parentId?: any; parentLevel?: number }>) {
  ...
  // Fixed taxonomy: Categoria (0) > Sub-Categoria (1) > Modelo (2).
  // What can be created here depends on the level of the parent category.
  const childLevel = parentId === undefined ? 0 : (parentLevel ?? 0) + 1;
  const canCreateChild = childLevel <= 2;
  const newCategoryTitle =
    childLevel === 1
      ? t`Nova Sub-Categoria`
      : childLevel === 2
        ? t`Novo Modelo`
        : t`Nova Categoria`;
  ...
  const tableActions = useMemo(() => {
    ...
    return [
      ...
      <AddItemButton
        ...
        tooltip={newCategoryTitle}
        onClick={() => newCategory.open()}
        hidden={!can_add || !canCreateChild}
      />
    ];
  }, [user, table.hasSelectedRecords, canCreateChild, newCategoryTitle]);
